...
int main(int argc, char *argv[]) {
    ...
    int minT = minTempHistorica(array);
    printf("\n La temperatura mínima histórica es  %d",minT);

    int maxTemp[YEARS];
    maxTempAnual(array,maxTemp);
    for (unsigned int year = 0u; year < YEARS; year++) {
        printf("\n La temperatura máxima del año %u fue de %d",year+FST_YEAR,maxTemp[year]);
    }

    month_t maxPrecAnual[YEARS];
    maxMesPrecAnual(array,maxPrecAnual);
    for (unsigned int year = 0u; year < YEARS; year++) {
        printf("\n El mes con mas precipitaciones del año %u fue %s",year+FST_YEAR, getMonthName(maxPrecAnual[year]));
    }
    printf("\n");
    return (EXIT_SUCCESS);
}
